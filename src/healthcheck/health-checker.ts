...
class HealthChecker {
  ...
  private async getPromiseStatus(statusResponse: HealthStatus) {
    ...
    this.startupPlugins.forEach((check) =>
  ...
  public async getReadinessStatus(): Promise<HealthStatus> {
    ...
    let filteredPromises = this.readinessPlugins.filter(
    ...
    if (filteredPromises.length === 0) {
      ...
      await Promise.all(
        filteredPromises.map(async (check) => {
  ...
  public async getLivenessStatus(): Promise<HealthStatus> {
    ...
    let filteredPromises = this.healthPlugins.filter(
      (element) => element !== undefined
    );
    if (filteredPromises.length === 0) {
      return statusResponse;
